...
export async function PATCH(request: NextRequest) {
  try {
    const { id, productId, variantId, qty } = await request.json();

    if (!id || !productId || !variantId || !qty) {
    ...
    const data = await FetchServer(API_ROUTE.cartId.replace(':id', id), {
      method: 'PATCH',
      body: JSON.stringify({ id, productId, variantId, qty }),
      auth: true,
    });
